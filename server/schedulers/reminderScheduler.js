            ...
            for (const farm of farms) {
                try {
                    ...
                    for (const v of vaccinations) {
                        ...
                        const item = { type: 'vaccination', title: `${v.medication || 'Vaccine'} — ${v.animal?.tagId || v.animal?.name || 'animal'}`, description: v.notes || '', dueDate: v.nextCheckup, days };
                    ...
                    for (const s of stockItems) {
                        if (s.minimumStock > 0 && s.quantity <= s.minimumStock) {
                            const item = { type: 'stock', title: `Low Stock: ${s.product}`, description: `${s.quantity} ${s.unit} remaining`, dueDate: new Date() };
                    ...
                    for (const i of invItems) {
                        if (i.lowStockAlert > 0 && i.quantity <= i.lowStockAlert) {
                            upcoming.push({ type: 'inventory', title: `Low: ${i.name}`, description: `${i.quantity} ${i.unit} remaining`, dueDate: new Date() });
                        }
                    }

                    const equipment = await Equipment.find({ farm: farm._id, nextMaintenance: { $ne: null, $lte: threeDays } });
                    for (const e of equipment) {
                        const due = new Date(e.nextMaintenance); due.setHours(0, 0, 0, 0);
                        const days = Math.ceil((due - today) / 86400000);
                        const item = { type: 'equipment', title: `Maintenance: ${e.name}`, dueDate: e.nextMaintenance, days };
                        if (days === 0) finalReminders.push(item);
                        else if (days <= 3) upcoming.push(item);
                    }

                    const tasks = await Task.find({ farm: farm._id, status: { $in: ['pending', 'in_progress'] }, dueDate: { $ne: null, $lte: threeDays } });
                    ...
                    if (finalReminders.length > 0) {
                        ...
                        if (farmer.phone && settings?.smsToggles?.farmerAlertHigh) {
                            await smsService.send(farmer.phone, 'farmerAlertHigh', {
                                user: farmer, message: `${finalReminders.length} items due today on ${farm.name}`, farmName: farm.name,
                            }).catch(() => {});
                        }
